...
int main(int argc, char const *argv[]) {
    ...
    double *z = calloc(N, sizeof *z); // Argument (1) number of blocks to be allocated, 2) size of each block in bytes.
    double *B = malloc(N * sizeof *B); // Argument is the number of bytes to allocate
    ...
    y = calloc(N, sizeof *y);
    z = calloc(N, sizeof *z); // sets memory to 0
...
// This solution does not cover all the values though.
// If we inspect the loop, we see that we are missing several values for each iteration of `j`.
// Six to be exact. E.g. when `j` is 2, we set values in `y` for $j = 2, 3, 4,$ and $5$,
// but $i$ is never incremented above 2. In the next iteration
// of the outer loop $j = 6, 7, 8,$ and $9$, and we never catch these values.
// To solve this we add the remaining values outside the inner loop, but still inside the outer one.
